...
whenever
	...
	and get_task_attribute_value(Domain,'domain')
	and member(Domain,['basic_needs', 'appreciating_culture', 'performing_producing_culture', 'physical_activities_sports', 'things_to_do_about_town'])
thenceforth
...
explanation_text('Recall that there were no requirements set w.r.t domains, values, social or physical closeness. Nevertheless, we tried to increase the gender diversity of selected users.',type1,_).
